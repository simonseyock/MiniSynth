...
synth.player.OneBarStepSequencer.prototype.addValue = function (stepIndex, value) {
	this.steps_[stepIndex].push(value);
};

synth.player.OneBarStepSequencer.prototype.removeValue = function (stepIndex, value) {
...
synth.player.OneBarStepSequencer.prototype.playNextBar = function (time, barLength) {
	...
	for( var i=0; i<this.steps_.length; i++) {
		for( var j=0; j<this.steps_[i].length; j++) {
			instrument.playFrequency(this.steps_[i][j], time + stepLength * i, stepLength, 100);
		}
	}
};
